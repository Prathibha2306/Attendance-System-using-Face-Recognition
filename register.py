...
def safe_open_image(rel_path, size, fallback_color=(0, 43, 83)):
...
class Register:
    def __init__(self,root):
        ...
        bg_full = safe_open_image(os.path.join("data_img", "app_bg.png"), (self.screen_w, self.screen_h))
        bg_full = bg_full.resize((self.screen_w, self.screen_h), Image.LANCZOS)
        self.bg=ImageTk.PhotoImage(bg_full)

        lb1_bg=Label(self.root,image=self.bg)
        lb1_bg.place(x=0,y=0, relwidth=1,relheight=1)

        # Center the sign-up card on the real screen (designed at 670x480)
        card_x = (self.screen_w - 670) // 2
        card_y = (self.screen_h - 480) // 2
        frame= Frame(self.root,highlightbackground="lightgray",highlightthickness=2,bg="#F2F2F2")
        frame.place(x=card_x,y=card_y,width=670,height=480)
